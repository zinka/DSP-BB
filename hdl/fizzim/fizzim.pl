...
&print($indent,"`default_nettype none\n\n");
...
if ($language eq "vhdl") {
  ...
  if (%parameters) { # If parameters are present, take care of them
    ...
    foreach $param (sort keys %parameters) {
      $param1 = uc($param);
      &print($indent,"parameter $param1 = $parameters{$param},\n");
    }
...
#######################
#&print($indent,"\n\n");
&print($indent,"${comm} state indices\n");
...
#######################
&print(0, "\n");
&print($indent,"${comm} state registers\n");
...
&print($indent,"// initial values\n");
# initial values for FPGAs
    foreach $regdp_output (sort keys %regdp_outputs) {
      ...
      &print($indent,"initial $regdp_output = $value;\n");
      &translate_last_line_to_vhdl_if_required("assignment");
    }
    &print($indent,"initial $statevar = $reset_state;\n");
...
&print($indent,"${comm} verilator lint_off CASEINCOMPLETE\n");
...
#      &print($indent, sprintf("$nextstatevar = ${total_statebits}\'b%s; ${comm} default to x because default_state_is_x is set\n","x"));
      &print($indent, sprintf("$nextstatevar = ${total_statebits}\'b%s; ${comm} default_state_is_x\n","x" x ${total_statebits}));
    } elsif ($language eq "systemverilog") {
...
if ($use_undefined_goto_state) {
  ...
  if ($encoding eq "heros" || $encoding eq "none") {
    &print($indent,"$nextstatevar = $undefined_goto_state; ${comm} undefined_states_go_here\n");
    &translate_last_line_to_vhdl_if_required("assignment");
  } elsif ($encoding eq "onehot") {
...
if ($language eq "vhdl") {
  ...
  &print(--$indent, "end\n");
  &print($indent,"${comm} verilator lint_on CASEINCOMPLETE\n");
}
...
&print($indent, "${comm} state sequential always block\n");
...
if (scalar(%regdp_outputs)) {
  ...
  &print($indent, "// verilator lint_off CASEINCOMPLETE\n");
...
&print($indent, "// verilator lint_on CASEINCOMPLETE\n");
#   &print($indent,"/*
#     ***************************************************************************
#     * user code
#     ***************************************************************************
...
if ($simcode && !($language eq "vhdl")) {
  ...
  &print($indent, "// verilator lint_off UNUSED\n");
  ...
  &print($indent, "// verilator lint_on UNUSED\n\n");
  &print(0, "// fizzim code generation ends\n\n");
}
...
# Handle top-of-file comments
if ($include_at_top_of_file2 = $globals{machine}{include_at_top_of_file2}{value}) {
  open(FILE,'+<', "$include_at_top_of_file2") or &warning($indent,"Could not open file $include_at_top_of_file specified by attribute include_at_top_of_file");
  while (<FILE>) {
    print;
  }
  print "endmodule";
